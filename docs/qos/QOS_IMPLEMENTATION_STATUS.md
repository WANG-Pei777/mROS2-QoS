...
| Durability | QoS field and SEDP representation for `VOLATILE`; partial `TRANSIENT_LOCAL` writer cache behavior exists. | `VOLATILE` is visible in ROS2 topic info. | `TRANSIENT_LOCAL` late-joiner behavior needs focused test coverage and cleanup rules. |
...
| Liveliness | AUTOMATIC setting and lease duration are represented; reader tracks heartbeat/activity; manual modes are rejected by QoS validation. | Focused activity and lease checks. | Liveliness lost/recovered state machine and manual liveliness semantics are not complete. |
...
Result directory: /home/your-user/mros2/mros2-esp32/results/qos_preflight_20260610_022457

Strict bidirectional RELIABLE reset stress validation: 3/3 PASS
Result directory: /home/your-user/mros2/mros2-esp32/results/qos_reset_stress_20260610_022750
...
The latest changes added reader receive-path statistics and a conservative early-data recovery path:
